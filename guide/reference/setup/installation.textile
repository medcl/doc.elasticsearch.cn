...
p. 从这里 "下载":/download 最新的版本并解压, 转到刚解压的目录， 通过如下命令来启动*elasticsearch*:
...
ElasticSearch 使用java编写, 所以你需要安装"Java":http://java.sun.com/javase/downloads/index.jsp 环境来运行. 并且确保设置好环境变量 @JAVA_HOME@ .
...
p. 启动shell脚本包含很多特性. 第一个, 最开始就说过了的, 能够很方便的控制elasticsearch服务到底是以前台还是后台的方式来运行.
...
p. 确保机器扩大了最大文件打开数（或者是运行elasticsearch服务的用户的文件打开数）. 强烈建议设置成32k 甚至是 64k .

p. 为了测试进程能够打开多少文件，可以通过传递参数 @-Des.max-open-files@ 并设置值为 @true@. 这样服务会在启动的时候打印进程能够打开的文件数目.
...
p. 服务使用的是Java Service Wrapper，一个很小的围绕java虚拟机的本地包装并且监视着虚拟机的各种状态.

p. 注意, 传递给JVM级别的参数配置 (如 -X 参数) 应该在配置文件 @elasticsearch.conf@ 里进行设置. 

p. 环境变量 @ES_MIN_MEM@ 和 @ES_MAX_MEM@ 用来设置JVM内存分配的参数 (单位 mega bytes). 分别是 @256@ 和 @1024@ .
