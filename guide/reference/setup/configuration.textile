...
在ES内部, 所有配置都会合并折叠为“基于命名空间的（namespaced）"的配置. 例如, 上面的配置会合并为 @network.host@. 这意味着elasticsearch可以灵活的支持其它格式的配置方式, 如, "JSON":http://www.json.org. 如果你比较钟爱json, 只要简单的将配置文件 @elasticsearch.yml@ 重命名为 @elasticsearch.json@ 并且内容改成:
...
p. 同时这也意味着可以很方便的通过外部参数传递的方式来提供这些设置，如 参数 @ES_JAVA_OPTS@ 同样也可作为 @elasticsearch@ 命令的参数, 如:
