...
Facets provide aggregated data . In the simplest case, a terms facet can return facet counts for various facet values for a specific field. ElasticSearch supports more facet implementations, such as statistical or date histogram facets. 有时要针对选定的facet项去过滤命中结果， all facets仍在原查询的基础上进行， @filter@ 适用于此场景。

p. a image about the difference between a facet and a carrot [by medcl]
