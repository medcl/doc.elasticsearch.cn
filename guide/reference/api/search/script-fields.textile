...
p. Script fields 可以着用于没有存储的域 ， 将返回自定义的值
...
p. Script fields 可以访问 the actual @_source@ document(indexed)  ,提取特定元素(包括对象)
...
p. 注意， 这里 @_source@ 指的事 json-like model. 
...
p.  使用doc方式会使这个域的词载入内存(cached), 缺点是耗内存； the @doc[...]@ 只能作用在简单值的域上，不能返回json对象 ，仅作用在不分词或单项的域有意义。
