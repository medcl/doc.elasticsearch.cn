...
p. Allows to return a "script evaluation":https://github.com/goog/doc.elasticsearch.cn/blob/master/guide/reference/modules/scripting.textile (based on different fields) for each hit, for example:
...
p. Script fields可用于存储为no的域 ，将返回自定义的值
...
p. Script fields 可以访问 @_source@ 源文件(indexed)  ,返回特定元素(包括对象 类型)
...
+注意+ ， 这里 @_source@ 指的是json-like model. 
...
**the difference between @doc['my_field'].value@ and @_source.my_field@.**
p.  使用doc方式会使这个域的词载入内存(cached), 缺点是耗内存； the @doc[...]@ 只能作用在简单域值，不能返回json对象 ，仅作用在不分析或单项的域有意义。
